...
function addReservationList(formData, i, k){
    ...
    for(var j = 0; j < reservationArray.get(roomIndex).length; j++){
        var parsedCheckInDate = reservationArray.get(roomIndex)[j].checkInDate.format("YYYY-MM-DD[T]HH:mm:ss");
        var parsedCheckOutDate = reservationArray.get(roomIndex)[j].checkOutDate.format("YYYY-MM-DD[T]HH:mm:ss");
        formData.append("roomList[" + k + "].reservationList[" + j + "].checkInDate", parsedCheckInDate);
        formData.append("roomList[" + k + "].reservationList[" + j + "].checkOutDate", parsedCheckOutDate);
    }
}